...
export async function POST(request) {
  ...
  try {
    const apiResponse = await forwardAuth('/auth/register', formData);
    if (!apiResponse.ok) return redirectTo('/auth/register?auth=failed');
    return redirectTo('/dashboard', apiResponse.headers.get('set-cookie'));
  } catch {
    return redirectTo('/auth/register?auth=unavailable');
  }
}
