#!/usr/bin/env python3
"""
Agent_Security_Selfcheck_v3.5.1.py
===================================
...
ACTIVE_COVERAGE: dict[str, Any] | None = None
...
def safe_read_text(path: Path, limit: int = 200_000) -> str:
    if path.name in SECRET_FILE_NAMES or path.name.startswith(".env"):
        if ACTIVE_COVERAGE is not None:
            ACTIVE_COVERAGE.setdefault("secret_files_not_read", []).append(str(path))
        return ""
    try:
        try:
            ...
            if ACTIVE_COVERAGE is not None:
                ACTIVE_COVERAGE.setdefault("decode_errors", []).append(str(path))
                ACTIVE_COVERAGE["security_relevant_incomplete"] = True
        if len(data) > limit and ACTIVE_COVERAGE is not None:
            file_kind = file_class(path, ACTIVE_ROOT) if ACTIVE_ROOT is not None and path.is_relative_to(ACTIVE_ROOT) else "external_runtime"
            if file_kind not in {"documentation", "test_fixture", "example", "scanner_artifact"}:
                ACTIVE_COVERAGE.setdefault("limit_overrides", []).append({"path": str(path), "requested_limit": limit, "read_full": True})
                return data
            ACTIVE_COVERAGE.setdefault("truncated_files", []).append({"path": str(path), "limit": limit})
        return data[:limit]
    except Exception as exc:
        if ACTIVE_COVERAGE is not None:
            if isinstance(exc, FileNotFoundError):
                ACTIVE_COVERAGE.setdefault("missing_optional_paths", []).append(str(path))
            else:
                ACTIVE_COVERAGE.setdefault("unreadable_paths", []).append({"path": str(path), "error": type(exc).__name__})
                if ACTIVE_ROOT is None or not path.is_relative_to(ACTIVE_ROOT) or file_class(path, ACTIVE_ROOT) not in {"documentation", "test_fixture", "example", "scanner_artifact"}:
                    ACTIVE_COVERAGE["security_relevant_incomplete"] = True
        return ""
...
def load_config(path: Path) -> dict[str, Any]:
    ...
    if yaml:
        try:
            ...
            if ACTIVE_COVERAGE is not None:
                ACTIVE_COVERAGE.setdefault("config_parse_errors", []).append({"path": str(path), "error": type(exc).__name__})
                ACTIVE_COVERAGE["security_relevant_incomplete"] = True
...
def count_known_config_dirs(root: Path, names: set[str], coverage: dict[str, Any] | None = None) -> dict[str, int]:
    counts = {name: 0 for name in names}
    for p in list_files(root, None, coverage):
        parts = set(p.parts)
        for name in names:
...
def classify_secret_matches(text: str, path: str = "") -> list[dict[str, Any]]:
    """Return redacted, line-scoped secret evidence without returning values."""
    matches: list[dict[str, Any]] = []
    assignment_literal = re.compile(r"(?i)(api[_-]?key|secret|token|password|passwd|authorization|bearer)\s*[:=]\s*['\"]([^'\"\s,]{8,})['\"]")
    env_reference = re.compile(r"(?i)(env\s*\([^)]*\)|\$\{[A-Z0-9_]+\}|\$[A-Z_][A-Z0-9_]*|process\.env\.[A-Z0-9_]+|os\.getenv\s*\(|secretRef|valueFrom)")
    for line_number, line in enumerate(text.splitlines(), start=1):
        literal_found = False
        for match in assignment_literal.finditer(line):
            ...
            matches.append({
                "path": path,
                "line": line_number,
                "kind": "confirmed_literal",
                "confidence": "high",
                "reason": "secret-shaped assignment with a literal value; value redacted",
            })
            literal_found = True
        if literal_found:
            continue
        for pattern in SECRET_PATTERNS[1:]:
            if pattern.search(line):
                matches.append({
                    ...
                    "kind": "provider_token",
                    "confidence": "high",
                    "reason": "provider-token-shaped literal; value redacted",
                })
                break
    return matches
...
def contains_secret_like_literal(text: str) -> bool:
    ...
    for line in text.splitlines():
        ...
        if any(ctx.lower() in low for ctx in benign_contexts):
            continue
        if any(match["kind"] in {"confirmed_literal", "provider_token"} for match in classify_secret_matches(line)):
            return True
    return False


def stable_finding_id(check: str, scope: str, path: str, line: int | None, evidence_kind: str) -> str:
    identity = "|".join([str(check), str(scope), str(path), str(line or 0), str(evidence_kind)])
    return f"finding-{hashlib.sha256(identity.encode('utf-8')).hexdigest()[:16]}"


def coverage_decision(coverage: dict[str, Any]) -> dict[str, Any]:
    incomplete = (
        not bool(coverage.get("scan_complete", True))
        or bool(coverage.get("security_relevant_incomplete"))
        or (coverage.get("scope_mode") in {"runtime", "full"} and bool(coverage.get("host_runtime_incomplete")))
    )
    return {
        "status": "ERROR" if incomplete else "PASS",
        "exit_code": 2 if incomplete else 0,
        "reason": "security-relevant coverage incomplete" if incomplete else "coverage complete",
    }


def runtime_probe_error(name: str, exc: BaseException | str) -> None:
    if ACTIVE_COVERAGE is None:
        return
    ACTIVE_COVERAGE.setdefault("runtime_probe_errors", []).append({"probe": name, "error": type(exc).__name__ if isinstance(exc, BaseException) else str(exc)})
    ACTIVE_COVERAGE["host_runtime_incomplete"] = True
    ACTIVE_COVERAGE["security_relevant_incomplete"] = True


def add(findings: list[Finding], category: str, check: str, status: str, risk: str,
        severity: int, evidence: str, recommendation: str, adapter: str = "core",
        *, scope: str = "target_source", confidence: str = "medium", reachability: str = "unknown",
        evidence_kind: str = "observation", source_path: str | None = None,
        source_line: int | None = None, derived: bool = False,
        derived_from: list[str] | None = None) -> None:
    if category in {"Runtime Environment", "System Resources"} and scope == "target_source":
        scope = "host_runtime"
        reachability = "host_observed"
    finding_id = stable_finding_id(f"{category}:{check}", scope, source_path or "", source_line, evidence_kind)
...
def find_repo_root(start: Path, discover: bool = False) -> Path:
    p = start.resolve()
    if not discover:
        return p
...
def list_files(root: Path, max_files: int | None = None, coverage: dict[str, Any] | None = None) -> list[Path]:
    ignored = {".git", "node_modules", ".venv", "venv", "__pycache__"}
    allowed_dot_dirs = {".github", ".mcp", ".config"}
    out: list[Path] = []
    def onerror(error: OSError) -> None:
        if coverage is not None:
            coverage.setdefault("walk_errors", []).append({"path": getattr(error, "filename", None), "error": type(error).__name__})
            coverage["security_relevant_incomplete"] = True

    for base, dirs, files in os.walk(root, onerror=onerror, followlinks=False):
        symlink_dirs = [name for name in dirs if (Path(base) / name).is_symlink()]
        for name in symlink_dirs:
            dirs.remove(name)
            if coverage is not None:
                coverage.setdefault("symlink_directory_skips", []).append(str(Path(base) / name))
                coverage["security_relevant_incomplete"] = True
        ...
        for name in files:
            ...
            if path.is_symlink():
                try:
                    ...
                    if coverage is not None:
                        coverage.setdefault("symlink_skips", []).append(str(path))
                    continue
            out.append(path)
            if max_files is not None and len(out) >= max_files:
                if coverage is not None:
                    coverage["truncated_file_list"] = True
                return out
    return out
...
def compose_state(path: Path) -> str:
    name = path.name.lower()
    if "example" in name or "sample" in name:
        return "example"
    if name in {"docker-compose.override.yml", "docker-compose.override.yaml", "compose.override.yml", "compose.override.yaml"}:
        return "active_override"
    if "update" in name or "override" in name:
...
def file_class(path: Path, root: Path) -> str:
    rel = str(path.relative_to(root)).lower()
    parts = set(Path(rel).parts)
    if "scanner-bundle" in parts or "graphify-out" in parts:
        return "scanner_artifact"
    if any(part in parts for part in {"test", "tests", "fixtures", "__tests__"}) or path.name.endswith(".test.ts") or path.name.endswith(".test.js"):
        return "test_fixture"
    if path.suffix.lower() in {".md", ".rst", ".txt"}:
        return "documentation"
    if "example" in path.name.lower() or "sample" in path.name.lower():
        return "example"
    if path.name in {"docker-compose.yml", "docker-compose.yaml", "compose.yml", "compose.yaml"} or "compose" in path.name.lower():
        return compose_state(path)
    return "production"


def discover_context(target: Path, scope_mode: str = "exact", runtime_root: Path | None = None,
                     compose_files: list[str] | None = None) -> dict[str, Any]:
    global ACTIVE_COVERAGE
    global ACTIVE_ROOT
    root = find_repo_root(target, discover=scope_mode == "discover")
    coverage: dict[str, Any] = {
        "requested_root": str(target),
        "effective_root": str(root),
        "scope_mode": scope_mode,
        "scan_complete": True,
        "security_relevant_incomplete": False,
        "runtime_probe_errors": [],
    }
    ACTIVE_COVERAGE = coverage
    ACTIVE_ROOT = root
    files = list_files(root, None, coverage)
    coverage["files_discovered"] = len(files)
    runtime_home = Path(os.environ.get("AGENT_HOME", str(Path.home())))
    runtime_base = (runtime_root or root).resolve()
    runtime_config_candidates = [
        runtime_base / "config.yaml",
        runtime_base / "agent.yaml",
        runtime_base / "runtime.yaml",
        runtime_base / "runtime.yml",
    ]
    if scope_mode in {"runtime", "full"}:
        runtime_config_candidates.extend([
    ...
    runtime_detected = bool(runtime_config or (runtime_base / "agent_cli").exists() or (runtime_base / "run_agent.py").exists())

    framework_markers = []
    for p in files:
        rel = str(p.relative_to(root)).lower()
        if file_class(p, root) == "scanner_artifact":
            continue
        # Framework adapters activate only on explicit configuration/manifest
        # evidence. Generic prose, middleware, routes, and POS terminology do
        # not establish an agent framework or tool backend.
        if p.name.lower() in {"framework.yaml", "framework.yml", "framework.json", "framework.toml", "agent.config.yaml", "agent.config.yml"}:
            framework_markers.append(str(p.relative_to(root)))
    docker_files = [str(p.relative_to(root)) for p in files if p.name == "Dockerfile" or ("compose" in p.name.lower() and p.suffix.lower() in {".yml", ".yaml"})]
    ...
    docker_config_files = [p for p in files if p.name == "Dockerfile" or ("compose" in p.name.lower() and p.suffix.lower() in {".yml", ".yaml"})]
    selected_compose_paths: list[Path] = []
    for raw_path in compose_files or []:
        ...
        try:
            ...
            coverage.setdefault("scope_errors", []).append({"path": str(selected), "reason": "compose selection outside target root"})
            coverage["security_relevant_incomplete"] = True
            continue
        if not selected.is_file():
            coverage.setdefault("scope_errors", []).append({"path": str(selected), "reason": "selected compose file missing"})
            coverage["security_relevant_incomplete"] = True
            continue
        selected_compose_paths.append(selected)
    selected_compose_set = {p.resolve() for p in selected_compose_paths}
    compose_configs = [{"path": str(p.relative_to(root)), "state": "active_selected" if p.resolve() in selected_compose_set else compose_state(p), "file_class": file_class(p, root)} for p in docker_config_files if p.name != "Dockerfile"]
    active_docker_config_files = [p for p in docker_config_files if p.name == "Dockerfile" or p.resolve() in selected_compose_set or compose_state(p) in {"active_candidate", "active_override"}]
    ...
    policy_files = [p for p in files if p.name in {"mcp.json", "tools.json", "toolsets.yaml", "toolsets.yml", "permissions.json", "policy.yaml", "policy.yml", "tool-policy.md"} or "policies" in p.parts]
    ...
    return {
        ...
        "coverage": coverage,
        "scope_mode": scope_mode,
        "excluded_paths": [str(p) for p in files if file_class(p, root) == "scanner_artifact"],
        ...
        "active_docker_config_files": active_docker_config_files,
        "compose_configs": compose_configs,
        "selected_compose_files": [str(p.relative_to(root)) for p in selected_compose_paths],
...
def scan_universal(ctx: dict[str, Any], findings: list[Finding]) -> None:
    ...
    contextual_secret_hits: list[dict[str, Any]] = []
    self_path = Path(__file__).resolve()
    for p in ctx["files"]:
        if p.resolve() == self_path or file_class(p, root) == "scanner_artifact":
            continue
        if p.name in SECRET_FILE_NAMES or p.name.startswith(".env"):
            continue
        if p.suffix.lower() not in {".yaml", ".yml", ".json", ".toml", ".ini", ".conf", ".md", ".py", ".js", ".ts", ".sh", ".bash"}:
            continue
        txt = safe_read_text(p, 100_000)
        for match in classify_secret_matches(txt, str(p.relative_to(root))):
            match["file_class"] = file_class(p, root)
            if match["file_class"] in {"production", "active_candidate", "optional_override", "development"}:
                secret_hits.append(match)
            else:
                contextual_secret_hits.append(match)
    secret_actionable = [match for match in secret_hits if match["kind"] in {"confirmed_literal", "provider_token"}]
    secret_status = "WARN" if secret_actionable else "SKIP" if contextual_secret_hits else "PASS"
    secret_risk = "High" if secret_actionable else "Info" if contextual_secret_hits else "Low"
    add(findings, "Secrets & Privacy", "No obvious plaintext secrets in non-secret configs",
        secret_status, secret_risk, 5 if secret_actionable else 1,
        f"literal_matches={secret_actionable}; contextual_matches={contextual_secret_hits}; values_redacted=true",
        "Move confirmed literals to approved secret storage and rotate anything that may have been exposed.",
        scope="target_source", confidence="high" if secret_actionable else "low", evidence_kind="secret_classification")
    ...
    host_runtime_collected = ctx["scope_mode"] in {"runtime", "full"}
    host_socket = Path("/var/run/docker.sock").exists() if host_runtime_collected else "not_collected"
    declared_socket = []
    active_socket = []
    for config in ctx["compose_configs"]:
        config_path = root / config["path"]
        text = safe_read_text(config_path, 120_000).lower()
        if "/var/run/docker.sock" in text:
            declared_socket.append({"path": config["path"], "state": config["state"]})
            if config.get("state") in {"active_candidate", "active_override", "active_selected"}:
                active_socket.append(config["path"])
    docker_evidence = f"host_socket_present={host_socket}; declared_socket_mounts={declared_socket}; active_candidate_socket_mounts={active_socket}; target_runtime_exposure=unknown"
    socket_scope = "target_source" if active_socket or not host_runtime_collected else "host_runtime"
    socket_reachability = "declared" if active_socket else "unknown" if not host_runtime_collected else "host_only"
    add(findings, "Container & Runtime", "Docker socket exposure reviewed",
        "WARN" if active_socket else "PASS", "High" if active_socket else "Info", 5 if active_socket else 1,
        docker_evidence,
        "Docker socket grants host-level control; mount only when explicitly required and reviewed.",
        scope=socket_scope, confidence="high", reachability=socket_reachability, evidence_kind="compose_socket_mount")

    docker_text = "\n".join(safe_read_text(p, 120_000) for p in ctx["active_docker_config_files"][:30]).lower()
    ...
    deploy_risky = []
    for p in ctx["deploy_files"]:
        if file_class(p, root) in {"documentation", "test_fixture", "example", "scanner_artifact"}:
            continue
    ...
    deploy_text = "\n".join(safe_read_text(p, 120_000) for p in ctx["deploy_files"] if file_class(p, root) not in {"documentation", "test_fixture", "example", "scanner_artifact"}).lower()
    ...
    executable_files = [p for p in ctx["files"] if p.suffix.lower() in {".sh", ".bash", ".py", ".js", ".ts", ".yaml", ".yml"} and file_class(p, root) not in {"documentation", "test_fixture", "example", "scanner_artifact"}]
    script_text = "\n".join(safe_read_text(p, 80_000) for p in executable_files).lower()
    ...
    add(findings, "Execution & Approvals", "Dangerous command exposure reviewed",
        ...
        f"dangerous_command_markers={dangerous_hits[:20]}; executable_files_scanned={len(executable_files)}; scan_complete={not bool(ctx['coverage'].get('truncated_file_list'))}",
    ...
    inventory_counts = count_known_config_dirs(root, {"plugins", "skills", "mcp", ".mcp", "servers"}, ctx["coverage"])
    ...
    availability = ({tool: bool(shutil.which(tool)) for tool in scanner_tools}
                    if ctx["scope_mode"] in {"runtime", "full"}
                    else {tool: "not_collected" for tool in scanner_tools})
...
def scan_runtime_environment(ctx: dict[str, Any], findings: list[Finding]) -> None:
    ...
    try:
        ...
        runtime_probe_error("proc_1_cgroup", exc)
    ...
    try:
        ...
        runtime_probe_error("systemd_detect_virt", exc)
    ...
    if not vm_type:
        for dmi_file in ["/sys/class/dmi/id/sys_vendor", "/sys/class/dmi/id/product_name"]:
            try:
                ...
                runtime_probe_error(f"dmi:{dmi_file}", exc)
    ...
    if not vm_type:
        try:
            ...
            runtime_probe_error("proc_cpuinfo_hypervisor", exc)
    ...
    try:
        ...
        runtime_probe_error("proc_version_wsl", exc)
    ...
    try:
        ...
        runtime_probe_error("os_release", exc)
...
def scan_system_resources(ctx: dict[str, Any], findings: list[Finding]) -> None:
    ...
    try:
        ...
        runtime_probe_error("proc_cpuinfo_count", exc)
    ...
    try:
        ...
        runtime_probe_error("proc_meminfo", exc)
    ...
    try:
        ...
        runtime_probe_error("statvfs_root", exc)
    ...
    try:
        ...
        runtime_probe_error("ip_interfaces", exc)
    ...
    try:
        ...
        runtime_probe_error("uname_kernel", exc)
...
def correlate(findings: list[Finding]) -> None:
    ...
    warn_fail = {f.check: f for f in findings if f.status in {"WARN", "FAIL"} and f.confidence in {"confirmed", "high"} and f.scope in {"target_source", "target_runtime"}}
    high_combo = []
    ...
    parents = [by_check[name].id for name in warn_fail if name in by_check]
    add(findings, "Risk Correlation", "Compound mesh risk correlated",
        "WARN" if high_combo else "PASS", "Critical" if high_combo else "Info", 8 if high_combo else 1,
        f"compound_risks={high_combo}; derived_from={parents}; total_warn_fail={sum(1 for f in findings if f.status in {'WARN','FAIL'})}",
        "Prioritize only confirmed same-scope compound risks; do not treat lexical co-occurrence as an attack chain.",
        scope="target_source" if high_combo else "external/unknown", confidence="high" if high_combo else "low",
        evidence_kind="correlation", derived=True, derived_from=parents)
    category_counts = {}
    for f in findings:
    ...
    add(findings, "Risk Correlation", "Cross-category audit coverage reviewed",
        ...
        "Coverage diagnostics are scanner metadata and do not change target risk scoring.",
        scope="external/unknown", confidence="high", evidence_kind="coverage_diagnostic")
...
REMEDIATION_PATHS = {
    ...
    "Framework deployment posture reviewed": "scripts/security-tools/agent-security-selfcheck/Agent_Security_Selfcheck_v3.5.1.py",
    ...
    "Risk Correlation / Cross-category audit coverage reviewed": "scripts/security-tools/agent-security-selfcheck/Agent_Security_Selfcheck_v3.5.1.py",
    "Persistence & Deployment / Framework persistence hooks reviewed": "scripts/security-tools/agent-security-selfcheck/Agent_Security_Selfcheck_v3.5.1.py",
}
...
def write_simple_pdf(path: Path, meta: dict[str, Any], res: dict[str, Any]) -> None:
    ...
    script_name = Path(str(meta.get("script_path", "Agent_Security_Selfcheck_v3.5.1.py"))).name
...
def severity_label(risk: str) -> str:
    return {
        "Critical": "critical",
        "High": "high",
        "Medium": "moderate",
        "Low": "low",
        "Info": "informational",
    }.get(str(risk), "informational")


def canonical_finding(finding: dict[str, Any]) -> dict[str, Any]:
    evidence = {
        "text": redact(finding.get("evidence", "")),
        "kind": finding.get("evidence_kind", "observation"),
        "scope": finding.get("scope", "target_source"),
        "confidence": finding.get("confidence", "medium"),
    }
    if finding.get("source_path"):
        evidence["path"] = finding["source_path"]
    if finding.get("source_line"):
        evidence["line"] = finding["source_line"]
    return {
        "id": finding.get("id") or stable_finding_id(finding.get("check", "finding"), finding.get("scope", "target_source"), "", None, "observation"),
        "title": finding.get("check", "Untitled finding"),
        "severity": severity_label(finding.get("risk", "Info")),
        "severityScore": int(finding.get("severity", 0)),
        "category": finding.get("category", "general"),
        "description": finding.get("recommendation", ""),
        "evidence": [evidence],
        "remediation": ((finding.get("remediation_task") or {}).get("instruction") or finding.get("recommendation")) if finding.get("status") in {"WARN", "FAIL"} else None,
        "references": finding.get("framework_controls", []),
        ...
        "evidenceKind": finding.get("evidence_kind", "observation"),
        "derived": bool(finding.get("derived", False)),
        "derivedFrom": finding.get("derived_from", []),
        "source": finding.get("source_path"),
    }


def target_asset_id(root: Path) -> str:
    return f"local-{hashlib.sha256(str(root).encode('utf-8')).hexdigest()[:16]}"
...
def write_reports(out_dir: Path, meta: dict[str, Any], res: dict[str, Any]) -> dict[str, str]:
    ...
    canonical = {
        ...
        "scanner": {
            ...
            "version": meta["script_version"],
            "scriptSha256": meta["script_sha256"],
        },
        "target": {
            "assetId": meta["target_asset_id"],
            "hostname": meta.get("target_hostname", "unknown"),
        },
        "findings": [canonical_finding(finding) for finding in res["findings"]],
        "collectedAt": meta["generated_utc"],
        "metadata": meta,
        "score": res["score"],
    ...
    json_path.write_text(json.dumps(canonical, indent=2), encoding="utf-8")
...
def git_meta(root: Path, collect: bool = True) -> dict[str, str]:
    if not collect:
        return {"git_commit": "not_collected", "git_dirty": "not_collected"}
...
def main() -> int:
    ...
    ap.add_argument("--scope-mode", choices=["exact", "discover", "runtime", "full"], default="exact", help="Scope selection; exact target is the fail-closed default")
    ap.add_argument("--runtime-root", default=None, help="Optional explicit host/runtime evidence root")
    ap.add_argument("--asset-id", default=None, help="Explicit producer target asset identity supplied by the trusted runner")
    ap.add_argument("--compose-file", action="append", default=[], help="Explicitly selected Compose file relative to --target; repeat for multiple files")
    ...
    ctx = discover_context(target, scope_mode=args.scope_mode, runtime_root=runtime_root, compose_files=args.compose_file)
    findings: list[Finding] = []
    scan_universal(ctx, findings)
    if args.scope_mode in {"runtime", "full"}:
        scan_runtime_environment(ctx, findings)
        scan_system_resources(ctx, findings)
    else:
        ctx["coverage"]["host_runtime_not_collected"] = True
    if args.scope_mode in {"runtime", "full"}:
    ...
    if args.scope_mode in {"runtime", "full"}:
        try:
            ...
            _raw_hw_data = collect_hardware_info()
            if _raw_hw_data.get("errors"):
                coverage.setdefault("runtime_probe_errors", []).extend({"probe": "hardware_collection", "error": redact(error)} for error in _raw_hw_data["errors"])
                coverage["host_runtime_incomplete"] = True
                coverage["security_relevant_incomplete"] = True
            _hw_data = sanitize_hardware(_raw_hw_data)
        except Exception as exc:
            _env_label = "unknown"
            _hw_data = {}
            runtime_probe_error("hardware_collection", exc)
    else:
        _env_label = "not_collected"
        _hw_data = {"scope": "host_runtime", "not_collected": True}

    coverage["scan_complete"] = not bool(coverage.get("security_relevant_incomplete"))
    if coverage.get("host_runtime_incomplete"):
    ...
    meta = {
        ...
        "target_asset_id": args.asset_id or target_asset_id(ctx["root"]),
        "target_hostname": os.environ.get("HOSTNAME", "unknown") if args.scope_mode in {"runtime", "full"} else "unknown",
        ...
        **git_meta(ctx["root"], collect=args.scope_mode != "exact"),
    }
    ...
    res = {
        ...
        "decision": coverage_decision(ctx["coverage"]),
    }
    ...
    if res["decision"]["exit_code"] != 0:
        return int(res["decision"]["exit_code"])
    if args.exit_zero:
        return 0
