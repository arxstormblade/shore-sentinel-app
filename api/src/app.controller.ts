...
export class AppController {
  ...
  @Post('auth/login') async login(@Body() body: Record<string, unknown>, @Res({ passthrough: true }) res: Response) { const result = await this.auth.login(requireString(body, 'email'), requireString(body, 'password')); this.setSessionCookie(res, result.token); return { user: result.user }; }
  ...
  private setSessionCookie(res: Response, token: string) { res.cookie('shore_session', token, { httpOnly: true, sameSite: 'lax', secure: process.env.NODE_ENV === 'production' }); }
