...
The live branch is `feat/enterprise-single-container` at commit `7e1f608`. Graphify was refreshed before this plan: Graphify `0.9.16` reported 1,999 nodes and 3,055 relationships for source HEAD `7e1f60867af5f15a26aaadbdfc20ee284874cc45`. The focused query connected `api/src/app.controller.ts`, `api/src/database.service.ts`, `api/src/queue.service.ts`, `web/components/ui.jsx`, `tests/test_side_navigation_shell.py`, `tests/test_deployment_hardening.py`, `tests/test_production_compose_release_gate.py`, and `docs/qa/2026-07-20-v1.1.0-release-scorecard.md`.
