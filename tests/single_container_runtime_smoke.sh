...
docker exec "$CONTAINER" sh -c "PGPASSWORD=\"\$POSTGRES_PASSWORD\" psql -h 127.0.0.1 -U \"\$POSTGRES_USER\" -d \"\$POSTGRES_DB\" -Atc 'SELECT count(*) FROM schema_migrations' | grep -qx 4"
...
docker restart -t 45 "$CONTAINER" >/dev/null
for attempt in $(seq 1 90); do
  if docker exec "$CONTAINER" /opt/shore-sentinel/bin/healthcheck.sh >/dev/null 2>&1; then break; fi
  [ "$attempt" -lt 90 ] || { docker logs "$CONTAINER"; exit 1; }
  sleep 2
